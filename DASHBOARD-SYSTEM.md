...
- **Accepting-clients start date** (label en UI; Script Property sigue siendo `CHALLENGE_START_DATE` vía `updateChallengeStartDate`). Es el día que **abre el promo / empiezas a aceptar clientes** (un viernes), **NO** el primer día del challenge. Hace dos cosas: (1) filtra qué participantes muestra el dashboard (`Date Purchased >= esta fecha`, en `getParticipantsFromSheet_`), y (2) el **Día 1 del challenge se calcula solo = esta fecha + 10 días** (`getDay1Date`, hardcode `+10`), así que "Challenge Day X" sale de ahí. Ej.: promo abre viernes 24-jul → Día 1 = lunes 3-ago. **Ojo:** poner el primer día del challenge (ej. 3-ago) rompe todo — excluye a los que compraron en la promo y empuja el Día 1 a +10. (Se renombró el label el 2026-08-03 para evitar esta confusión; el `+10` sigue hardcodeado.)
...
- **Call time** (`renderCallTimeSection`): si el participante está en `CC - Scheduled` o `DC - Scheduled`, muestra la **hora real de la call en Eastern** (de GHL Calendar, `ccCallAt`/`dcCallAt`). Si la call terminó hace 1h+ y sigue en Scheduled, se pinta ámbar con ⚠ "still marked Scheduled". Config compartida `CALL_TYPES` (CC/DC).
...
18. ~~**Scheduled Calls — fase CC pendiente.**~~ ✅ **Hecho (2026-08-03):** `getCcCallTimesByEmail_` (dos calendarios CC, dedup por id, cache `ccCallTimesByEmail:v1`, calentado dentro de `warmLastDcCache`) + `ccCallAt`/`ccCallEndAt`/`ccCallAssignedUserName` en `getOpportunities`. Frontend generalizado a CC+DC (`CALL_TYPES`, `getAllScheduledCalls`). Requiere pegar `Code.gs` + New Version.
